...
'''    if User.objects.filter(email = email, password = password).exists():
        user=User.objects.get(email = email, password = password)
    elif Therapist.objects.filter(email = email, password = password).exists():
        user = Therapist.objects.get(email = email, password = password)
    elif Lawyer.objects.filter(email = email, password = password).exists():
        user = Lawyer.objects.get(email = email, password = password)
    elif Ngo.objects.filter(email=email, password=password).exists():
        user = Ngo.objects.get(email=email, password=password)
    else:
        return JsonResponse({'error': 'Invalid username or password'}, status=400)'''
